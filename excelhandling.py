...
import openpyxl
path = "Company.xlsx"
s = openpyxl.load_workbook(path)
sheet = s.active
print(sheet["A1"].value)

for cell_row in range(1, sheet.max_row + 1):
    cell_range = f"A{cell_row}:C{cell_row}"
    join_val = []
    for row in sheet[cell_range]:
        for cell in row:
            if cell.value is not None:
                join_val.append(str(cell.value))

    cell_join = "-".join(join_val)
    left = cell_range.split(":")[0]

    sheet[left] = cell_join
